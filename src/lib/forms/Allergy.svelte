<script lang="ts">
    ...
    let refines = 1;
    let defaults = ["years", "months", "days"]
</script>

<mb-form>
    {#each [...Array(risks)] as _, i}
        <div
            class="border shadow-lg p-5 rounded-lg flex flex-col gap-3 mt-5 md:p-10"
        >
            {#if i > 0}
                <div class="flex flex-row-reverse font-medium">
                    <sl-button
                        ...
                        ><sl-icon slot="prefix" name="dash-square-fill" />Delete
                        Allergy</sl-button
                    >
                </div>
            {/if}
            <p>Duration</p>
            {#each [...Array(refines)] as _,i} 
            <mb-quantity default={defaults[i]}>
                <mb-unit unit="years" label="Years" />
                <mb-unit unit="months" label="Months" />
                <mb-unit unit="days" label="Days" />
            </mb-quantity>
            {/each}
            <div>
                <sl-button type="neutral" on:click={()=>refines++}>Refine</sl-button>
            </div>
            <!-- <mb-input
